...
public int[] parseTag(Object context, XSSFSheet sheet, XSSFRow curRow, XSSFCell curCell) {
    ...
    while (it.hasNext()) {
      ...
      if (ExcelUtils.isCanShowType(value)) {
        ...
        XSSFCell cell = WorkbookUtils.getCell(curRow, eachPos);
        ...
        if (width > 1) {
          XSSFCell nextCell = WorkbookUtils.getCell(curRow, eachPos + 1);
        ...
        for (int rownum = curRow.getRowNum(); rownum < curRow.getRowNum() + rowMerged; rownum++) {
          for (int i = 0; i < width; i++) {
            XSSFRow r = WorkbookUtils.getRow(rownum, sheet);
            XSSFCell c = WorkbookUtils.getCell(r, eachPos + i);
            XSSFCell cc = WorkbookUtils.getCell(r, curCell.getColumnIndex());
        ...
        if (width > 1 || rowMerged > 1) {
          sheet.addMergedRegion(new CellRangeAddress(curRow.getRowNum(), curRow.getRowNum() + rowMerged - 1, cell.getColumnIndex(),
