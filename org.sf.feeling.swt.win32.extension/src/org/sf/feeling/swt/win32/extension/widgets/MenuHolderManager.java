		...
		Display.getDefault().addFilter(SWT.KeyDown, new Listener()
		{
			...
			public void handleEvent(Event e)
			{
				...
				if (map != null && map.contains(shortcut)) {
					...
					if (map.getMenuItem(shortcut).isEnabled())
						map.getMenuItem(shortcut).fireSelectionEvent(e);
				}
			}
		});
	}
		...
		for (int i = 0; i < menu.getItemCount(); i++) {
			...
			if (item.getMenu() != null) {
				...
				if (item.getShortcut() != null)
					map.putShortcut(item.getShortcut(), item);
			}
		}
	}
	...
	private static void removeShortcut(ShortcutMap shortcutMap, CMenu menu) {
		for (int i = 0; i < menu.getItemCount(); i++) {
			...
			if (item.getMenu() != null) {
				...
				if (item.getShortcut() != null)
					shortcutMap.removeShortcut(item.getShortcut());
			}
		}
	}
}
