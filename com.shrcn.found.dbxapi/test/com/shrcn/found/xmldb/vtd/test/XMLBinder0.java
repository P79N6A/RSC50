...
class XMLBinder0 {
	...
	private String getSubString(VTDNav vn, String attName) throws XPathParseException {
		AutoPilot ap = createAP(vn, attName);
		return ap.evalXPathToString();
	}
	
	private double getSubNumber(VTDNav vn, String attName) throws XPathParseException {
		AutoPilot ap = createAP(vn, attName);
		return ap.evalXPathToNumber();
	}
	
	private int getSubInt(VTDNav vn, String attName) throws XPathParseException {
		return (int) getSubNumber(vn, attName);
	}

	private AutoPilot createAP(VTDNav vn, String attName)
...
//            ap4.bind(vn);
            while(ap0.evalXPath()!=-1){
                ...
                cdr.title = getSubString(vn, "TITLE/aaa");
                cdr.artist = getSubString(vn, "ARTIST");
                cdr.price = getSubNumber(vn, "PRICE");
                cdr.year = getSubInt(vn, "YEAR");
