 ...
 *
 */
public class SignalDAO implements SCLDAO {
	 ...
	 * @return
	 */
	public static List<Element> queryOuterIeds(String iedName, EnumCtrlBlock signalType) {
		String xpath = "/scl:SCL/scl:IED[@name!='" + iedName + "'][count(./scl:AccessPoint/scl:Server/scl:LDevice/*/scl:" + signalType.name() + ")>0]";
	 ...
	 * @return
	 */
	public static List<Element> queryIedDataSets(String iedName, EnumCtrlBlock signalType) {
		List<Element> list = new ArrayList<Element>();
		Element iedNode = IEDDAO.getIEDNode(iedName);
		List<Element> cbEls = DOM4JNodeHelper.selectNodes(iedNode, "./scl:AccessPoint/scl:Server/scl:LDevice/*/scl:" + signalType.name());
		Element iedCpEl = DOM4JNodeHelper.createRTUNode("IED");
		for (Element cbEl : cbEls) {
			...
			Element apCpEl = DOM4JNodeHelper.selectSingleNode(iedCpEl, "./AP[@name='" + apName + "']");
			if (apCpEl == null) {
				apCpEl = iedCpEl.addElement("AP");
				DOM4JNodeHelper.copyAttributes(apEl, apCpEl);
				list.add(apCpEl);
			}
			
			String ldInst = ldEl.attributeValue("inst");
			Element ldCpEl = DOM4JNodeHelper.selectSingleNode(apCpEl, "./LDevice[@inst='" + ldInst + "']");
			if (ldCpEl == null) {
