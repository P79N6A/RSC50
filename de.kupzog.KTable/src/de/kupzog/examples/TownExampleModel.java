...
public class TownExampleModel implements KTableModel {
	...
	private final ImageCellRenderer m_exampleRender = new ImageCellRenderer(120);
...
//			return content[row - 1];
			if (col == 0)
				return content[row - 1].image;
			else if (col == 1)
				return content[row - 1].country;
			else
				return content[row - 1].notes;
		}
	}
